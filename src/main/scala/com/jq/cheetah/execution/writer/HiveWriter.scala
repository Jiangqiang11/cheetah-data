...
import com.jq.cheetah.CheetahConf
import com.jq.cheetah.util.TimeUtils
...
/**
  ...
  */
private[cheetah] class HiveWriter(@transient val sparkSession: SparkSession, val conf: CheetahConf) extends Writer{
  ...
  override def write(inputDataFrame: DataFrame): Unit = {
    ...
    val tableColumns = sparkSession.sql(s"DESC ${table}")
    ...
    def writeByMode(): Unit = {
      conf.getExtractType match {
        case conf.EXTRACT_TYPE_FULL => {
          val dataFrame = sparkSession.createDataFrame(inputDataFrame.rdd, tableSchema)
        ...
        case conf.EXTRACT_TYPE_INCREMENT => {
          val dataFrameNum = sparkSession.sparkContext.accumulator(0, s"${table}数据累加器")
          ...
          val dataFrame: DataFrame = sparkSession.createDataFrame(inputDataFrame.rdd, tableSchema)

//          val dataFrame = hiveContext.createDataFrame(inputDataFram, tableSchema)
          val writer = dataFrame.write.format("parquet").mode(SaveMode.Overwrite)
